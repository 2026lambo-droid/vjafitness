...
export default function ServicesSection({ data }) {
  ...
  const getBgImage = (url, i) => {
    const src = failedUrls[i] ? IMAGE_FALLBACK : url;
    return `linear-gradient(180deg, rgba(0,0,0,${hoveredIndex === i ? 0.55 : 0.7}), rgba(0,0,0,${hoveredIndex === i ? 0.55 : 0.7})), url(${src})`;
  };
      ...
      <div className="container">
        ...
        <div className="cs_card_1_group" data-aos="fade-right">
          {data.service.map((service, index) => {
            ...
            const overlayAlpha = isActive ? 0.55 : 0.7; // slightly lighter on active card
            ...
            return (
              <div
                ...
                style={{
                  backgroundImage: getBgImage(service.image, index),
                }}
              ...
              >
                <img
                  ...
                  onError={() => setFailedUrls((p) => ({ ...p, [index]: true }))}
                />
                <div className="cs_card_top">
                  <div className="cs_card_tags">
                    {service.tags.map((tag, i) => (
                      <Link key={i} to={tag.url} className="cs_card_tag">
                        {tag.label}
                      </Link>
                    ))}
                  </div>
