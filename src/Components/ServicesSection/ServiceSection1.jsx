...
export default function ServiceSection1({ data }) {
  ...
  const getImgSrc = (url, index) => (failedUrls[index] ? IMAGE_FALLBACK : url);
  const handleError = (index) => setFailedUrls((p) => ({ ...p, [index]: true }));
            ...
            <div className="col-lg-6" key={index}>
              <div className="cs_card cs_style_2">
                <Link to={service.link} className="cs_card_thumb">
                  <img
                  src={getImgSrc(service.imgSrc, index)}
                  alt={service.title || "VJAFitness - Servicio"}
