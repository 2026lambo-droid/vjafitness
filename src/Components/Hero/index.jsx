...
export default function HeroSection({ data }) {
  ...
  const [failedBg, setFailedBg] = useState({});
  const getBg = (src, i) => (failedBg[i] ? IMAGE_FALLBACK : src);
        ...
        <div className="container">
          <div className="cs_hero_in">
            <div className="cs_hero_text">
              <h1
                className="cs_hero_title cs_fs_80 cs_normal cs_mb_16 cs_white_color wow fadeInUp"
                data-aos="fade-up"
                dangerouslySetInnerHTML={{ __html: data.title }}
              />

              <div className="cs_hero_copy_wrap">
                <p
                  className="cs_hero_subtitle cs_fs_24 cs_mb_16 cs_white_color cs_hero_subtitle_weight"
                  dangerouslySetInnerHTML={{ __html: data.subtitle }}
                ></p>
                {data.credibility && (
                  <p className="cs_hero_credibility cs_fs_16 mb-0 cs_white_color cs_opacity_8">
                    {data.credibility}
                  </p>
                )}
              </div>
            ...
            >
              <div className="cs_hero_funfact">
                {data.funfact.trustIndicators && (
                  <div className="cs_hero_funfact_indicators">
                    {data.funfact.trustIndicators.map((item, index) => (
                      <div
                        key={index}
                        style={{
                          display: "flex",
                      ...
                      >
                        ...
                        />
                        <span className="cs_white_color cs_hero_funfact_text">
                          {item.text}
                        </span>
                      </div>
                    ))}
                ...
                <SwiperSlide key={index}>
                  <div className="cs_slide">
                    <img
                      ...
                      src={getBg(src, index)}
                      alt="VJAFitness - Entrenamiento glúteos y pierna"
                      onError={() => setFailedBg((p) => ({ ...p, [index]: true }))}
                      style={{
                        width: "100%",
                        objectFit: "cover",
