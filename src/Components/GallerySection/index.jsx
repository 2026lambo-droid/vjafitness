...
export default function GallerySection({ data }) {
  ...
  const getSrc = (src, i) => (failedIdx[i] ? IMAGE_FALLBACK : src);
        ...
        <div className="cs_gallery cs_style_2">
          {data?.map((item, index) => (
            <div
              ...
              style={{
                backgroundImage: `url(${getSrc(item.imageSrc, index)})`,
              }}
              key={index}
      ...
      <div className="container">
        <div className="cs_gallery_grid_1">
          {data?.map((item, index) => (
            ...
            >
              <img
                src={getSrc(item.imgSrc, index)}
                alt={item.title || "VJAFitness - Transformación fitness"}
                onError={() => setFailedIdx((p) => ({ ...p, [index]: true }))}
              />
